...
async function upsertUserInfo(): Promise<string> {
...
function sendAdProfile(data: {
  user_uid: string
  sda_profile: {
    user: { data: { name: string; ext: { segtax: number }; segment: { id: string }[] }[] }
    content: { data: { name: string; ext: { segtax: number }; segment: { id: string }[] }[] }
  }
}) {
  try {
...
async function onCheckEnabledMessageHandled(
  ...
  const enabled = message.url && !disable_domains.includes(message.url) && enabledExtension
